...
* ``settings.txt``: Is not an usable settings file, just a dummy file for base
  backend tests;
* ``settings.json``: Is not an usable settings file, its values are dummy
  pointing to path and files that does not exists;
* ``settings_polluted.json``: Is almost equivalent of ``settings_custom.json``
