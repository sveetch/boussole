...
def test_self(settings, inspector):
    ...
    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_0))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_0))


def test_direct(settings, inspector):
    """
    Ensure direct circular imports are detected
    """
    ...
    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_1))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_1))

    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_2))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_2))


def test_indirect(settings, inspector):
    """
    Ensure indirect circular imports are detected
    """
    ...
    # Case 3
    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_3))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_3))

    # Case 4
    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_4))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_4))

    # Bridge
    with pytest.raises(CircularImport):
        list(inspector.children(sourcepath_bridge))

    with pytest.raises(CircularImport):
        list(inspector.parents(sourcepath_bridge))
...
# def test_004_subimport(settings, inspector):
    # """Ensure sub circular imports are detected"""
    # sourcepath_3 = os.path.join(settings.sample_path, 'main_circular_3.scss')
    # sourcepath_4 = os.path.join(settings.sample_path, 'main_circular_4.scss')
    # sourcepath_bridge = os.path.join(settings.sample_path, 'main_circular_bridge.scss')
    # sourcepath_5 = os.path.join(settings.sample_path, 'main_circular_5.scss')
